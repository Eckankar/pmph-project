...
void   run_cuda(
   ...
   //REAL u[outer][numY][numX];
    ...
    for (int j = numT-2; j>=0; --j) {
        ...
        rollback_implicit_x_part2_kernel<<<GRID(numY, outer), block_size2>>>(outer, numX, numY, numZ, u_t_d,
                a_d, b_d, c_d, yy_d); // 2D
        ...
        rollback_implicit_y_part2_kernel<<<GRID(numX, outer), block_size2>>>(outer, numX, numY, numZ, numT, j, myTimeline_d,
